...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Instructions and Recent Activities Row -->
        <div style="display: flex; gap: 24px; margin-bottom: 32px;">
            <!-- Instructions Box -->
            <div class="instructions-container" style="width: 60%; height: 70vh; float: none; margin: 0;">
                <div class="instructions-header">
                    <h3><i class="fas fa-bullhorn"></i> Instructions</h3>
                </div>
                <div class="instructions-image">
                    <?php
                    // Get all uploaded instruction images
                    ...
                    if (is_dir($instructions_dir)) {
                        ...
                        foreach ($files as $file) {
                            if ($file != '.' && $file != '..' && preg_match('/\.(jpg|jpeg|png|gif)$/i', $file)) {
                                $instruction_images[] = $instructions_dir . $file;
                            }
                        }
                    }
                    ...
                    if (empty($instruction_images)) {
                        $instruction_images[] = 'images/instructions/default-instructions.jpg';
                    }
                    ?>
                    
                    <div class="client-instructions-container">
                        <div class="instructions-slider">
                            <?php foreach ($instruction_images as $index => $image): ?>
                                <div class="instructions-slide <?= $index === 0 ? 'active' : '' ?>">
                                    <img src="<?= $image ?>" alt="Site Instructions" class="instructions-img">
                                </div>
                            <?php endforeach; ?>
                            
                            <?php if (count($instruction_images) > 1): ?>
                                <button class="instructions-nav prev" onclick="changeSlide(-1)">
                                    <i class="fas fa-chevron-left"></i>
                                </button>
                                <button class="instructions-nav next" onclick="changeSlide(1)">
                                    <i class="fas fa-chevron-right"></i>
                                </button>
                                
                                <div class="instructions-dots">
                                    <?php foreach ($instruction_images as $index => $image): ?>
                                        <span class="instructions-dot <?= $index === 0 ? 'active' : '' ?>" onclick="currentSlide(<?= $index + 1 ?>)"></span>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                        </div>
            ...
            <!-- Recent Activities -->
            <div class="dashboard-graph" style="width: 40%; height: 70vh; float: none; margin: 0; display: flex; flex-direction: column;">
                ...
                <div style="flex: 1; overflow-y: auto; display: flex; flex-direction: column;">
                    <?php if (count($recent_activities) > 0): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; flex: 1; padding: 40px; color: #666;">
    ...
    <script>
        ...
        // Instructions slider functionality
        let slideIndex = 1;
        const slides = document.querySelectorAll('.instructions-slide');
        const dots = document.querySelectorAll('.instructions-dot');
        const prevBtn = document.querySelector('.instructions-nav.prev');
        const nextBtn = document.querySelector('.instructions-nav.next');
