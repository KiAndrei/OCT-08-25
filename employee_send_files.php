...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
        case 'get_document_data':
            ...
            $stmt = $conn->prepare("SELECT document_data FROM client_request_form WHERE id = ?");
        ...
        case 'generate_pdf':
            ...
            // Get document data
            $stmt = $conn->prepare("SELECT document_data, request_id FROM client_request_form WHERE id = ?");
            $stmt->bind_param("i", $file_id);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            if ($row = $result->fetch_assoc()) {
                ...
                try {
                    ...
                    $update_stmt = $conn->prepare("UPDATE client_request_form SET pdf_file_path = ?, pdf_filename = ? WHERE id = ?");
        ...
        case 'approve_document':
            $file_id = intval($_POST['file_id']);
            $stmt = $conn->prepare("UPDATE client_request_form SET status = 'Approved' WHERE id = ?");
            $stmt->bind_param("i", $file_id);
        ...
        case 'reject_document':
            $file_id = intval($_POST['file_id']);
            $reason = $_POST['reason'] ?? '';
            $stmt = $conn->prepare("UPDATE client_request_form SET status = 'Rejected', document_data = CONCAT(document_data, ',\"rejection_reason\":\"', ?, '\"') WHERE id = ?");
            $stmt->bind_param("si", $reason, $file_id);
        ...
        case 'download_pdf':
            $file_id = intval($_POST['file_id']);
            $stmt = $conn->prepare("SELECT pdf_file_path, pdf_filename FROM client_request_form WHERE id = ?");
            $stmt->bind_param("i", $file_id);
            $stmt->execute();
            $result = $stmt->get_result();
        ...
        case 'debug_data':
            ...
            // Get document data
            $stmt = $conn->prepare("SELECT document_data, request_id, document_type FROM client_request_form WHERE id = ?");
            $stmt->bind_param("i", $file_id);
            $stmt->execute();
            $result = $stmt->get_result();
        ...
        case 'delete_document':
            try {
                ...
                $stmt = $conn->prepare("SELECT request_id, document_type, client_id FROM client_request_form WHERE id = ?");
                $stmt->bind_param("i", $file_id);
                $stmt->execute();
                $result = $stmt->get_result();
                ...
                if ($row = $result->fetch_assoc()) {
                    ...
                    // Delete the document
                    $delete_stmt = $conn->prepare("DELETE FROM client_request_form WHERE id = ?");
...
function generateSoloParentHTML($data) {
    ...
    // Build HTML mirroring the Solo Parent TCPDF layout
...
$stmt = $conn->prepare("
    ...
    WHERE crf.document_type IS NOT NULL AND crf.document_type != ''
...
// Debug: Check records with document types
$debug_stmt2 = $conn->prepare("SELECT COUNT(*) as total FROM client_request_form WHERE document_type IS NOT NULL AND document_type != ''");
$debug_stmt2->execute();
...
try {
    $debug_stmt3 = $conn->prepare("SELECT id, request_id, client_id, document_type, document_data, pdf_file_path, pdf_filename, submitted_at FROM client_request_form ORDER BY id DESC LIMIT 10");
    $debug_stmt3->execute();
    ...
    $debug_stmt3 = $conn->prepare("SELECT id, request_id, client_id, document_type FROM client_request_form ORDER BY id DESC LIMIT 10");
    $debug_stmt3->execute();
    $debug_res3 = $debug_stmt3->get_result();
    $recent_records = [];
    while ($row = $debug_res3->fetch_assoc()) {
                    ...
                    <div class="empty-state">
                        ...
                        <div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-radius: 8px; font-size: 0.9rem;">
                            ...
                            <?php foreach ($recent_records as $record): ?>
                                ID: <?= $record['id'] ?>, Request: <?= $record['request_id'] ?>, Type: <?= isset($record['document_type']) ? $record['document_type'] : 'N/A' ?>, Data: <?= isset($record['document_data']) ? 'Yes' : 'No' ?>, Submitted: <?= isset($record['submitted_at']) ? $record['submitted_at'] : 'N/A' ?><br>
                            <?php endforeach; ?>
                        </div>
                    </div>
                <?php else: ?>
                            ...
                            <div class="file-card" data-status="<?= strtolower($file['file_status']) ?>">
                                ...
                                <div class="file-actions">
                                    ...
                                    <?php if (isset($file['pdf_file_path']) && !empty($file['pdf_file_path']) && file_exists($file['pdf_file_path'])): ?>
                                        <a href="<?= htmlspecialchars($file['pdf_file_path']) ?>" target="_blank" class="btn btn-info" title="View existing PDF">
                                            <i class="fas fa-eye"></i>
                                            View PDF
                                        </a>
                                        <button onclick="downloadPDF(<?= $file['id'] ?>)" class="btn btn-success" title="Download PDF file">
